...
$(document).on("page-change",function(){frappe.ui.form.close_grid_form();});},show_doc:function(route){var dt=route[1],dn=route.slice(2).join("/"),me=this;if(frappe.model.new_names[dn]){dn=frappe.model.new_names[dn];frappe.set_route("Form",dt,dn);return;}
...
if(!this.setup_done)this.setup();cur_frm.cscript.is_onload=false;if(!this.opendocs[this.docname]){cur_frm.cscript.is_onload=true;this.setnewdoc();}else{this.render_form(is_a_different_doc);}
...
_f.Frm.prototype.add_custom_button=function(label,fn,icon,toolbar_or_class){this.page.add_inner_button(label,fn);}
...
if(me.frm.meta.allow_rename&&me.frm.perm[0].write){this.page.add_menu_item(__("Rename"),function(){me.frm.rename_doc();},true);}
...
new Sortable($rows.get(0),{handle:".sortable-handle",onUpdate:function(event,ui){me.frm.doc[me.df.fieldname]=[];$rows.find(".grid-row").each(function(i,item){var doc=$(item).data("doc");doc.idx=i+1;$(this).find(".row-index").html(i+1);me.frm.doc[me.df.fieldname].push(doc);});me.frm.dirty();}});},get_data:function(){var data=this.frm.doc[this.df.fieldname]||[];data.sort(function(a,b){return a.idx-b.idx});return data;},set_column_disp:function(fieldname,show){if($.isArray(fieldname)){var me=this;for(var i=0,l=fieldname.length;i<l;i++){var fname=fieldname[i];frappe.meta.get_docfield(me.doctype,fname,me.frm.docname).hidden=show?0:1;}}else{frappe.meta.get_docfield(this.doctype,fieldname,this.frm.docname).hidden=show?0:1;}
