...
this.make({page:this.parent.page,method:'frappe.desk.reportview.get',get_args:this.get_args,parent:this.page.main,start:0,show_filters:true,new_doctype:this.doctype,allow_delete:true,});this.make_delete();this.make_column_picker();this.make_sorter();this.setup_print();this.make_export();this.set_init_columns();this.make_save();this.make_user_permissions();this.set_tag_and_status_filter();this.page.add_menu_item(__("Refresh"),function(){me.refresh();},true);},set_init_columns:function(){var columns=frappe.defaults.get_default("_list_settings:"+this.doctype);if(!columns){var columns=[['name',this.doctype],];$.each(frappe.meta.docfield_list[this.doctype],function(i,df){if((df.in_filter||df.in_list_view)&&df.fieldname!='naming_series'&&!in_list(frappe.model.no_value_type,df.fieldtype)){columns.push([df.fieldname,df.parent]);}});}
...
return order_by.join(", ");},get_selected_table_and_column:function(select){return select.selected_doctype?this.get_full_column_name([select.selected_fieldname,select.selected_doctype]):"";},get_full_column_name:function(v){if(!v)return;return(v[1]?('`tab'+v[1]+'`'):this.tab_name)+'.`'+v[0]+'`';},build_columns:function(){var me=this;return $.map(this.columns,function(c){var docfield=frappe.meta.docfield_map[c[1]||me.doctype][c[0]];if(!docfield){var docfield=frappe.model.get_std_field(c[0]);docfield.parent=me.doctype;if(c[0]=="name"){docfield.options=me.doctype;}}
coldef={id:c[0],field:c[0],docfield:docfield,name:__(docfield?docfield.label:toTitle(c[0])),width:(docfield?cint(docfield.width):120)||120,formatter:function(row,cell,value,columnDef,dataContext,for_print){var docfield=columnDef.docfield;docfield.fieldtype={"_user_tags":"Tag","_comments":"Comment","_assign":"Assign","_starred_by":"StarredBy",}[docfield.fieldname]||docfield.fieldtype;if(docfield.fieldtype==="Link"&&docfield.fieldname!=="name"){docfield.link_onclick=repl('frappe.container.page.reportview.set_filter("%(fieldname)s", "%(value)s").page.reportview.run()',{fieldname:docfield.fieldname,value:value});}
...
open_url_post(frappe.request.url,args);},true);},make_save:function(){var me=this;if(frappe.user.is_report_manager()){this.page.add_menu_item(__('Save'),function(){if(me.docname){var name=me.docname}else{var name=prompt(__('Select Report Name'));if(!name){return;}}
return frappe.call({method:'frappe.desk.reportview.save_report',args:{name:name,doctype:me.doctype,json:JSON.stringify({filters:me.filter_list.get_filters(),columns:me.columns,sort_by:me.sort_by_select.val(),sort_order:me.sort_order_select.val(),sort_by_next:me.sort_by_next_select.val(),sort_order_next:me.sort_order_next_select.val()})},callback:function(r){if(r.exc){msgprint(__("Report was not saved (there were errors)"));return;}
if(r.message!=me.docname)
frappe.set_route('Report',me.doctype,r.message);}});},true);}},make_delete:function(){var me=this;if(this.can_delete){$(this.parent).on("click","input[type='checkbox'][data-row]",function(){me.data[$(this).attr("data-row")]._checked=this.checked?true:false;});this.page.add_menu_item(__("Delete"),function(){var delete_list=[]
...
return;if(frappe.confirm(__("This is PERMANENT action and you cannot undo. Continue?"),function(){return frappe.call({method:'frappe.desk.reportview.delete_items',args:{items:delete_list,doctype:me.doctype},callback:function(){me.refresh();}});}));},true);}},make_user_permissions:function(){var me=this;if(this.docname&&frappe.model.can_set_user_permissions("Report")){this.page.add_menu_item(__("User Permissions Manager"),function(){frappe.route_options={doctype:"Report",name:me.docname};frappe.set_route("user-permissions");},true);}},});frappe.ui.ColumnPicker=Class.extend({init:function(list){this.list=list;this.doctype=list.doctype;},clear:function(){this.columns=[];$(this.dialog.body).html('<div class="text-muted">'+__("Drag to sort columns")+'</div>\
   ...
   <div><button class="btn btn-default btn-add"><i class="icon-plus"></i>\
    '+__("Add Column")+'</button></div>');},show:function(columns){var me=this;if(!this.dialog){this.dialog=new frappe.ui.Dialog({title:__("Pick Columns"),width:'400',primary_action_label:__("Update"),primary_action:function(){me.update_column_selection();}});this.dialog.$wrapper.addClass("column-picker-dialog");}
...
this.report_ajax=frappe.call({method:"frappe.desk.query_report.run",type:"GET",args:{"report_name":me.report_name,filters:filters},callback:function(r){me.report_ajax=undefined;me.make_results(r.message.result,r.message.columns);}});return this.report_ajax;},trigger_refresh:function(){var me=this;var filters=me.get_values();var missing=false;$.each(me.filters,function(k,_f){if(_f.df.reqd&&!filters[_f.df.fieldname]){missing=true;return;}});if(!missing){me.refresh();}},get_values:function(raise){var filters={};var mandatory_fields=[];$.each(this.filters||[],function(i,f){var v=f.get_parsed_value();if(v==='%')v=null;if(f.df.reqd&&!v)mandatory_fields.push(f.df.label);if(v)filters[f.df.fieldname]=v;})
...
return filters;},make_results:function(result,columns){this.wrapper.find(".waiting-area, .no-report-area").empty().toggle(false);this.wrapper.find(".results").toggle(true);this.make_columns(columns);this.make_data(result,columns);this.render(result,columns);},render:function(result,columns){this.columnFilters={};this.make_dataview();this.id=frappe.dom.set_unique_id(this.wrapper.find(".result-area").addClass("slick-wrapper").get(0));this.grid=new Slick.Grid("#"+this.id,this.dataView,this.columns,this.slickgrid_options);this.grid.setSelectionModel(new Slick.CellSelectionModel());this.grid.registerPlugin(new Slick.CellExternalCopyManager({dataItemColumnValueExtractor:function(item,columnDef,value){return item[columnDef.field];}}));this.setup_header_row();this.grid.init();this.setup_sort();if(this.get_query_report_opts().tree){this.setup_tree();}},make_columns:function(columns){var me=this;var formatter=this.get_formatter();this.columns=[{id:"_id",field:"_id",name:__("Sr No"),width:60}].concat($.map(columns,function(c){if($.isPlainObject(c)){var df=c;}else if(c.indexOf(":")!==-1){var opts=c.split(":");var df={label:opts.length<=2?opts[0]:opts.slice(0,opts.length-2).join(":"),fieldtype:opts.length<=2?opts[1]:opts[opts.length-2],width:opts.length<=2?opts[2]:opts[opts.length-1]};if(df.fieldtype.indexOf("/")!==-1){var tmp=df.fieldtype.split("/");df.fieldtype=tmp[0];df.options=tmp[1];}
df.width=cint(df.width);}else{var df={label:c,fieldtype:"Data"};}
if(!df.fieldtype)df.fieldtype="Data";if(!cint(df.width))df.width=80;var col=$.extend({},df,{label:df.label||(df.fieldname&&__(toTitle(df.fieldname.replace(/_/g," "))))||"",sortable:true,df:df,formatter:formatter});col.field=df.fieldname||df.label;df.label=__(df.label);col.name=col.id=col.label=df.label;return col}));},get_query_report_opts:function(){return frappe.query_reports[this.report_name]||{};},get_formatter:function(){var formatter=function(row,cell,value,columnDef,dataContext,for_print){var value=frappe.format(value,columnDef.df,{for_print:for_print},dataContext);if(columnDef.df.is_tree){value=frappe.query_report.tree_formatter(row,cell,value,columnDef,dataContext);}
...
this.page.add_menu_item(__("Print"),function(){frappe.render_grid({grid:me.grid,title:me.page.title})},true);this.filter_inputs.range&&this.filter_inputs.range.on("change",function(){me.refresh();});if(this.setup_plot_check)
...
return false;},});;frappe.templates["print_grid"] = ' {% if(title) { %} <h2>{%= __(title) %}</h2> <hr> {% } %} <table class="table table-bordered">  <thead> <tr> {% var columns_length = columns.length; for (var i=0; i < columns_length; i++) { var col = columns[i]; %} {% if(col.name && col._id !== "_check") { %} <th style="min-width: {%= col.minWidth %}px" {% if(col.docfield && in_list(["Float", "Currency", "Int"], col.docfield.fieldtype)) { %} class="text-right" {% } %}>{%= __(col.name) %}</th> {% } %} {% } %} </tr> </thead>  <tbody> {% for (var i=0, l= data.length; i < l; i++) { var row = data[i]; %} <tr> {% for(var c=0; c < columns_length; c++) { var col = columns[c]; %} {% if(col.name && col._id !== "_check") { %} <td>{%= col.formatter ? col.formatter(i, c, row[col.id], col, row, true) : row[col.id] %}</td> {% } %} {% } %} </tr> {% } %} </tbody> </table> ';
